# ELURA Pitch Deck V4 — 19 slides

![ELURA Pitch Deck V4 contact sheet](/Users/shin/elura/biz/pitchdeck/final-16x9/contact-sheet.png)

## Slides

1. Cover
2. Problem
3. Activated Desire
4. Core Hypothesis
5. Why Art First
6. Solution
7. Core Experience
8. Result
9. Customer
10. Acquisition Wedge
11. Market & Expansion
12. Business Model
13. Validation
14. Go to Market
15. Competition
16. Defensibility
17. Roadmap
18. Founder & Current Ask
19. Closing

All slides are `1672 × 941 px`, exact 16:9.

The full content and evidence boundary is documented in [DECK_BRIEF.md](./DECK_BRIEF.md). The cross-deck critique and correction log is in [AUDIT_V2.md](../AUDIT_V2.md).

V1 is preserved in `../final-16x9-v1/`.
